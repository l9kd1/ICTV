...
def proxy_web_ctx_processor(handler):
    flask.request.host = flask.request.env.get('HTTP_X_FORWARDED_HOST', flask.request.host)
    flask.request.ip = flask.request.env.get('HTTP_X_FORWARDED_FOR', flask.request.ip)
    flask.request.homedomain = '%s://%s' % (flask.request.protocol, flask.request.host)
    return handler()
...
def get_app(config, sessions_path=""):
    ...
    app.add_url_rule('/logs', view_func=LogsPage.as_view('logs_page'))
